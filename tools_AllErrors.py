...
def clicked31(): 
    ...
    if model1 or model2 or model3 or model4 or model5 or model6 or model7 or model8 or model9 or model10 or model11 or model12 or model13 or model14 or model15 is True: # IMRT
        for field_number in range (num_beam): 
            ...
            if treatment is False:
                ...
                df = pd.DataFrame(dataset,columns =['CP_LN','CP','LN','MU','DICOM-MLC Position','MLC_velocity','MLC_acceleration',
                                                    'Cumulative % of MU','Beam On/Off','Gravity Vector-MLC','Gravity Vector-Gantry','Bank'])
                
                """df2 = pd.DataFrame(dataset,columns =['CP','MU','MLC_velocity','MLC_acceleration','Cumulative % of MU',
                                                                'Gravity Vector-MLC','Gravity Vector-Gantry','Bank'])"""
                li.append(df)
                ...
                # Dose Rate (This is more like whether the beam is on or off)
                dose_rate=[["Beam On/Off"],[0]*120] 
                for j in range(1,cp):
                    dose_rate_cp=[]
                    for m in range(120):
                        doseratecp=1
                ...
                df = pd.DataFrame(matrix_t,columns =['CP_LN','CP','LN','MU','DICOM-MLC Position','MLC_velocity','MLC_acceleration',
                                                    'Cumulative % of MU','Beam On/Off','Gravity Vector-MLC','Gravity Vector-Gantry','Bank'])
                """df2 = pd.DataFrame(matrix_t,columns =['CP','MU','MLC_velocity','MLC_acceleration','Cumulative % of MU',
                                                     'Gravity Vector-MLC','Gravity Vector-Gantry','Bank'])"""
                li.append(df)
        ...
        for field_number in range (num_beam): # VMAT
            ...
            if treatment is False:
                ...
                # Gantry Velocity at CP - Only VMAT
    ...
    for field_number in range (num_beam): 
        ...
        if treatment is False:
            ...
            global y1
            if model1 or model6 or model11 is True:
                X = li[field_number][['CP','MU','MLC_velocity','MLC_acceleration','Cumulative % of MU',
                                     'Gravity Vector-MLC','Gravity Vector-Gantry','Bank']].values.reshape(-1,8)
                y1 = clf.predict(X)
                #y1 = [x for xs in y1 for x in xs]
                #y1 = np.array(y1)
             
            elif model16 or model21 or model26 is True:
                X = li[field_number][['CP','MU','MLC_velocity','MLC_acceleration','Cumulative % of MU',
                                     'Gravity Vector-MLC','Gravity Vector-Gantry','Gantry_velocity','Gantry_acceleration',
                                     'Bank']].values.reshape(-1,10)
                y1 = clf.predict(X)
                #y1 = [x for xs in y1 for x in xs]
                #y1 = np.array(y1)
                
            elif model2 or model3 or model4 or model5 or model7 or model8 or model9 or model10 or model12 or model13 or model14 or model15 is True:
                X = li[field_number][['CP','MU','MLC_velocity','MLC_acceleration','Cumulative % of MU',
                                     'Gravity Vector-MLC','Gravity Vector-Gantry','Bank']].values.reshape(-1,8)
                y1 = clf.predict(X)
